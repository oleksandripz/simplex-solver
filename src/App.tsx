...
export default function App() {
  ...
  // Функція для швидкого завантаження прикладу з вашої курсової роботи
  ...
  // Хендлери для зміни розмірності
  ...
  // Розумний компилятор у стандартну форму для Симплекс-методу
  const solveProblem = () => {
    setError(null);
    try {
      // 1. Валідація даних
      ...
      if (cMain.some(isNaN)) {
        throw new Error("Будь ласка, заповніть усі коефіцієнти цільової функції числовими значеннями.");
      }
      ...
      const parsedConstraints = constraints.map((c, idx) => {
        ...
        if (coeffs.some(isNaN) || isNaN(rhs)) {
          throw new Error(`Помилка в обмеженні №${idx + 1}: заповніть усі поля числами.`);
        }
        if (rhs < 0) {
          throw new Error(`Вільний член (b) в обмеженні №${idx + 1} є від'ємним. Симплекс-метод вимагає b >= 0.`);
        }
        return { coeffs, sign: c.sign, rhs };
      });

      // 2. Розрахунок додаткових змінних
      ...
      // 4. Побудова розширеної цільової функції (вектор C)
      const cReal = Array(totalCols).fill(0);
      const cM = Array(totalCols).fill(0);

      // Записуємо основні змінні
      for (let j = 0; j < numVars; j++) cReal[j] = cMain[j];

      // 5. Побудова розширеної матриці A та вектора b
      ...
      parsedConstraints.forEach((c, i) => {
        ...
        // Копіюємо основні коефіцієнти
        for (let j = 0; j < numVars; j++) {
          A_matrix[i][j] = c.coeffs[j];
        }
      ...
      // Перетворюємо масиви в об'єкти DualNumber (r + m*M)
      const cDual = cReal.map((r, idx) => new DualNumber(r, cM[idx]));

      // 6. Запуск обчислювального двигуна
      const solver = new SimplexSolver();
      const resultSteps = solver.solve(cDual, A_matrix, b_vector, basisIndices, varNames, artificialVarsIndices);
      ...
      if (e instanceof Error) {
        ...
        setError("Сталася невідома помилка під час обчислень.");
      }
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6 max-w-7xl mx-auto text-gray-800">
      <header className="mb-8 p-6 bg-gradient-to-r from-blue-700 to-indigo-800 text-white rounded-2xl shadow-md">
        <h1 className="text-3xl font-extrabold mb-2">Універсальний Симплекс-Калькулятор (М-Метод)</h1>
        <p className="opacity-90">Введіть власні коефіцієнти лінійного програмування для пошуку мінімуму витрат.</p>
        <div className="mt-4 flex gap-4">
          <button
            onClick={loadDefaultExample}
            className="px-4 py-2 bg-white bg-opacity-20 hover:bg-opacity-30 rounded-lg font-semibold transition"
          >
            Завантажити дані з курсової
          </button>
        </div>
      </header>
      ...
      <section className="mb-6 p-4 bg-white rounded-xl shadow-sm border flex flex-wrap gap-6 items-center">
        <div>
          <label className="block text-xs font-bold uppercase text-gray-500 mb-1">Кількість змінних (n)</label>
          <input
            type="number"
            value={numVars}
            onChange={(e) => handleDimensionChange(parseInt(e.target.value) || 1, numConstraints)}
            className="w-24 p-2 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none"
          />
        </div>
        <div>
          <label className="block text-xs font-bold uppercase text-gray-500 mb-1">Кількість обмежень (m)</label>
          <input
            type="number"
            value={numConstraints}
            onChange={(e) => handleDimensionChange(numVars, parseInt(e.target.value) || 1)}
            className="w-24 p-2 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none"
          />
        </div>
      </section>

      {/* Введення Математичної Моделі */}
      <section className="grid grid-cols-1 gap-6 mb-8">
        {/* Цільова функція */}
        <div className="p-6 bg-white rounded-xl shadow-sm border">
          <h2 className="text-lg font-bold mb-4 text-blue-900">Коефіцієнти цільової функції (Мінімізація Z)</h2>
            ...
            {objective.map((val, idx) => (
              <div key={idx} className="flex items-center gap-1">
                <input
                  ...
                  className="w-16 p-2 text-center border rounded-lg font-mono focus:bg-blue-50 outline-none"
                />
                <span className="text-sm font-semibold text-gray-500">x{idx + 1} {idx < numVars - 1 ? '+' : ''}</span>
              </div>
            ))}
            <span className="text-blue-700 font-bold ml-2">→ min</span>
          </div>
        </div>

        {/* Обмеження */}
        <div className="p-6 bg-white rounded-xl shadow-sm border overflow-x-auto">
          <h2 className="text-lg font-bold mb-4 text-blue-900">Система обмежень (Умови-нерівності)</h2>
            ...
            {constraints.map((constraint, i) => (
              <div key={i} className="flex items-center gap-2 bg-gray-50 p-2 rounded-lg">
                ...
                {constraint.coeffs.map((val, j) => (
                  <div key={j} className="flex items-center gap-1">
                    <input
                      ...
                      className="w-14 p-1.5 text-center border rounded font-mono outline-none focus:bg-indigo-50"
                    />
                ...
                <select
                  ...
                  onChange={(e) => {
                    const next = [...constraints];
                    next[i].sign = e.target.value as '<=' | '>=' | '=';
                    setConstraints(next);
                  }}
                  className="mx-2 p-1.5 border rounded bg-white font-bold text-gray-700 outline-none cursor-pointer"
                >
                ...
                <input
                  ...
                  className="w-20 p-1.5 text-center border rounded font-bold font-mono outline-none bg-red-50 focus:bg-red-100"
                />
              </div>
            ))}
          </div>
      ...
      {/* Кнопка розрахунку та помилки */}
      <div className="mb-8">
        <button
          onClick={solveProblem}
          className="px-8 py-3 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl shadow-md transition transform active:scale-95"
        >
          Розрахувати кроки симплекс-методу
        </button>
        {error && (
          <div className="mt-4 p-4 bg-red-100 text-red-800 rounded-xl font-medium border border-red-200">
            {error}
          </div>
        )}
      </div>

      {/* Відображення покрокового розв'язання */}
      {steps.length > 0 && (
        <section className="space-y-6">
          <h2 className="text-2xl font-extrabold text-gray-900 border-b pb-2">Покрокове розв'язання математичної моделі</h2>
          {steps.map((step, idx) => (
            <TableauView key={idx} tableau={step} />
          ))}
        </section>
      )}
    </div>
